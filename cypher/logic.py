...
def key_schedule(key):
    ...
    logs.append({'key': key})
    logs.append({'key_length': len(key)})

    binary_key = buffer_to_binary(to_ascii_code(b64_key))
    logs.append({'binary_key': binary_key})
    logs.append({'binary_key_length': len(binary_key)})

    keys = []
    ins = 1
    while len(keys) < 52:
        round_logs = {'round_{}'.format(ins): []}
        for index in range(15, len(binary_key), 16):
            round_logs['round_{}'.format(ins)].append(binary_key[index - 15:index + 1])
            keys.append(binary_key[index - 15:index + 1])
        binary_key = binary_key[25:len(binary_key)] + binary_key[0:25]
        ins = ins + 1
        logs.append(round_logs)
    return {'keys': keys, 'logs': logs}
...
def cypher_half_round(entries, keys):
    ...
    return {
        'out': step_1 + step_2 + step_3 + step_4,
        'logs': logs
    }
...
def cypher_process(block, keys):
    ...
    logs.append({'cypher_round_1': entries})
    for x in range(0, 8):
        out = cypher_round(entries, keys, x)
        entries = out['entries']
        logs.append({
            'cypher_round_{}'.format(x + 2): entries,
            'steps': out['logs']
        })
    output = cypher_half_round(entries, keys)
    logs.append({'half_cypher_round': entries, 'steps': output['logs']})
    return {'output': output['out'], 'logs': logs}
...
def cypher_bit_block(block_array, keys):
    ...
    for block in block_array:
        cypher_process_result = cypher_process(block, keys)
        cypher_array.append(cypher_process_result['output'])
        logs.append({
            'block': block,
            'logs': cypher_process_result['logs']
        })
    return {
        'cypher_array': cypher_array,
        'logs': logs
    }
...
def encrypt(text, keys):
    ...
    logs.append({'text_b64': text_b64})
    ascii_buffer = to_ascii_code(text_b64)
    logs.append({'ascii_buffer': ascii_buffer})
    binary_buffer = buffer_to_binary(ascii_buffer)
    logs.append({'binary_buffer': binary_buffer})
    bit64_array = binary_plaintext_to_64bit_array(binary_buffer)
    logs.append({'bit64_array': bit64_array})
    cr = cypher_bit_block(bit64_array, keys)
    cypher_array = cr['cypher_array']
    logs.append({'cypher_array': cypher_array, 'logs': cr['logs']})
    cypher_binary_text = ""
    ...
    logs.append({'ascii_cypher_buffer': ascii_cypher_buffer})
    cypher_text = to_string(ascii_cypher_buffer)
    return {
        'cypher_text': str_to_b64(cypher_text),
        'logs': logs
    }
...
def decription_keys_scheduling(encription_keys):
    ...
    logs.append({'decryption_key': prepared_encription_keys})
    logs.append({'decryption_key_length': len(prepared_encription_keys)})
    ...
    logs.append({'decryption_key': decription_keys})
    logs.append({'decryption_key_length': len(decription_keys)})
...
def decrypt(text, keys):
    ...
    decrypted_array = cypher_bit_block(bit64_array, keys)["cypher_array"]
